...
const topItems = [
  { icon: Union, label: "Invoices", path: "/invoice" },
  { icon: Frame, label: "AI Invoices", path: "/" },
...
export const Sidebar = () => {
  ...
  // ✅ Footer click handler (Settings yahan navigate karega)
        ...
        {topItems.map((item) => {
          ...
          return (
            <button
              key={item.label}
              type="button"
              onClick={() => handleActivate(item.label, item.path)}
              onKeyDown={(e) => {
        ...
        <div className="flex flex-col items-center gap-1 mb-3">
          {footerItems.map((item) => (
            <button
              key={item.label}
              type="button"
              onClick={() => handleFooterClick(item)} // ✅ navigate for Settings
